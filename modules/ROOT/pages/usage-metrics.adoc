...
:description: Learn how to monitor and track usage metrics in MuleSoft to measure resource consumption, API calls, and platform activity across your organization.
:keywords: usage metrics, mulesoft monitoring, api usage, resource consumption, analytics, platform metrics, usage tracking, mulesoft analytics
...
|RPA	
|Robotic Process Automation (âRPAâ) bot minutes: The number of minutes running process automations across all bot sessions 
...
Sideloaded apps for hybrid standalone deployments are shown with the â icon in usage reports. Redeployments of sideloaded apps result in multiple records appearing in usage reports.
...
To calculate usage, MuleSoft meters and aggregates the number of messages daily and monthly. After a message is triggered, the report doesn't track changes to the message because the message is processed within the applicationâs flows.
...
APIkit is a tool that simplifies API implementation by automatically generating a minimal set of Mule flows based on the API specification. Each APIkit router endpoint counts as a distinct Mule flow. These Mule flows donât have an event source and are used to HTTP requests for a particular API method and path.
...
Mule flows that don't have an event source and aren't used for routing APIkit requests arenât charged against your Anypoint Platform package allotment. These Mule flows are primarily used to modularize code.
...
Note that the CPU capacity of a single node is derived from the `Allocatable:cpu:` field when you describe the node. The total cluster CPU capacity is the sum of all these `Allocatable:cpu:` values in millicores from the worker nodes. These worker nodes are those that donât have these labels `node-role.kubernetes.io/master`, `node-role.kubernetes.io/control-plane`, `node-role.kubernetes.io/ingress`, or `rtf.mulesoft.com/ingress`.
