...
| vCore Min
| Guaranteed base vCore allocation for the application, based on the minimum replica configuration. Applies to all CloudHub 2.0 applications regardless of whether HPA is enabled. For CloudHub (CH1) applications, vCore Min equals vCore Peak.
|===
...
For CloudHub 2.0 applications:

* The *vCore Min* value represents the guaranteed base vCore allocation.
* The *vCore Peak* value reflects actual consumption during the reporting period. When HPA is enabled, vCore Peak can exceed vCore Min during scaling events.
