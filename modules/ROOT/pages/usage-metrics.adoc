...
:keywords: usage metrics, pricing metrics, anypoint mq usage, api experience hub, api governance, api manager
...
These table defines usage metrics by product and explains how each metric is calculated for reporting.
...
|Calculated based on the peak allocated virtual cores assigned to active CloudHub (CH1) and CloudHub 2.0 (CH2) application workers in production environments.
|vCore Usage in Preproduction: Virtual processing capacity allocated to run Mule applications and APIs in non-production environments, such as sandbox, QA, staging, and development
|Calculated based on the peak allocated virtual cores assigned to active CloudHub (CH1) and CloudHub 2.0 (CH2) application workers in non-production environments.
...
.3+|Model Proxy	
| Prompt tokens: Number of tokens a Model Proxy in the prompts a Model Proxy sends to a specific LLM Model
| These tokens are the sum of all prompt tokens a specific Model Proxy sends to a specific LLM Model during a month. A prompt sent to a Model Proxy are considered an Omni Gateway API call and count towards the Omni Gateway API call metric.

| Completion tokens: Number of tokens in the responses that a Model Proxy receives from a specific LLM Model
|These tokens are the sum of all response tokens a specific Model Proxy receives from a specific LLM Model during a month.

| Total tokens: Total number of tokens used in interactions between a Model Proxy and a specific LLM Model
...
Model Proxy usage reports show this information in tables and cards:

Model Proxy:: 
Name of the Model Proxy

Business Group:: 
Business group where the Model Proxy is deployed

Client app:: 
The client application or integration that sent requests to the Model Proxy
...
Prompt tokens:: 
The sum of tokens in prompts the Model Proxy sends to the model in the given time period

Completion tokens:: 
The sum of tokens in responses the Model Proxy receives from the model in the given time period

Total tokens:: 
Total tokens used in interactions between the Model Proxy and the model in the given time period. This includes prompt tokens, completion tokens, and any provider-specific thinking tokens when applicable.
...
Select the *Usage by Business Group* tab to view:
...
|vCore Peak | Highest hourly vCore consumption for applications in the selected business group and time period.
|===

==== Usage by Application

Select the *Usage by Application* tab to view:
...
| HPA Status | Indicates whether Horizontal Pod Autoscaling (HPA) is enabled for the application. Displays Enabled or N/A.

| vCore Peak | Highest hourly vCore consumption for the application within the selected time period.

| vCore Min | Guaranteed base vCore allocation for the application, based on the minimum replica configuration. For CloudHub (CH1) applications, vCore Min equals vCore Peak.

| Mule Flows | Total number of flows in the Mule app, calculated by multiplying flows by the number of workers (CH1) or replicas (CH2 and RTF).
