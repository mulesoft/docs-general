...
|Environment Type |The environment in which a Mule app is deployed. Preproduction environments in the usage table refer to sandbox environments.

| Cluster |The name of the Runtime Fabric instance containing the applications, workers, and nodes. 
...
| CPU Limit (Millicores) | The maximum amount of CPU allocated by the user that the application can burst to within the shared cluster or Runtime Fabric instance.

| CPU Reserve (Millicores) | The amount of CPU allocated by the user to reserve for applications withing the Runtime Fabric cluster or instance. 
