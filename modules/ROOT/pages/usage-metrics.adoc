...
| vCore Min
| For CloudHub 2.0 applications with HPA enabled, the guaranteed base vCore allocation is based on the minimum replica configuration. For applications without HPA (CloudHub and CloudHub 2.0 non-HPA apps), vCore Min equals vCore Peak.
|===

[NOTE]
The vCore Min can be higher than vCore Peak when the guaranteed base allocation exceeds actual consumption during the reporting period.
...
* The *vCore Min* value represents the guaranteed base allocation.
* The *vCore Peak* value reflects the highest actual consumption, which can exceed the base allocation during scaling events.

HPA is available to customers who are opted into the new pricing and packaging model.
