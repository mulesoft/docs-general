...
| vCore Min | Guaranteed base vCore allocation for the application, based on the minimum replica configuration. Applies to all CloudHub 2.0 applications regardless of whether HPA is enabled. For CloudHub (CH1) applications, *vCore Min* equals *vCore Peak*.
...
The *vCore Peak* and *vCore Min* columns display combined vCore consumption across CloudHub (CH1) and CloudHub 2.0 (CH2) applications. vCore usage is sampled every hour; the peak hourly value is used to calculate the daily and monthly maximum:
