...
.3+|Model Proxy	
| Prompt tokens: Number of tokens an Model Proxy in the prompts an Model Proxy sends to a specific LLM Model
| These tokens are the sum of all prompt tokens a specific Model Proxy sends to a specific LLM Model during a month. A prompt sent to an Model Proxy are considered an Omni Gateway API call and count towards the Omni Gateway API call metric.

| Completion tokens: Number of tokens in the responses that an Model Proxy receives from a specific LLM Model
|These tokens are the sum of all response tokens a specific Model Proxy receives from a specific LLM Model during a month.

| Total tokens: Total number of tokens used in interactions between an Model Proxy and a specific LLM Model
