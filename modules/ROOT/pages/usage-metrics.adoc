...
| vCore Min a| Guaranteed base vCore allocation for the application, based on the minimum replica configuration. Applies to all CloudHub 2.0 applications regardless of whether HPA is enabled. For CloudHub (CH1) applications, vCore Min equals vCore Peak.

[NOTE]
vCore Min can be higher than vCore Peak when the guaranteed base allocation exceeds actual consumption during the reporting period.
...
vCore usage columns (*vCore Peak* and *vCore Min*) display the combined vCore consumption across CloudHub (CH1) and CloudHub 2.0 (CH2) applications.
...
For CloudHub 2.0 applications with HPA enabled, an application can scale beyond its base allocation, so *vCore Peak* can exceed *vCore Min* during scaling events.
...
[[max-cloudhub-vcore]]
==== Maximum CloudHub vCore Usage

*Maximum CloudHub vCore Usage (CH1/CH2 Combined)* shows the highest peak vCore consumption across all CloudHub and CloudHub 2.0 applications in the organization, displayed with the date and time (UTC) of the peak. The card appears in both production and preproduction views.
