...
Watch this video to see how to discover services with providers and scanners:

++++
include::_partials/exp-scanners-add-from-providers-video.html[]
++++
...
[[kong-gateway-scanner-protocol-to-asset-type-mapping]]
=== Protocol to Asset Type Mapping

When the Kong Gateway scanner discovers a service, it assigns an asset type based on the service's Kong protocol and plugins.
...
[NOTE]
====
Services that use other protocols or plugins, such as WebSocket or Simple Object Access Protocol (SOAP), are registered as asset type REST for now.
====

[[kong-gateway-scanner-rescanning-behavior]]
=== Rescanning Behavior

The Kong Gateway scanner assigns an asset type to a service the first time it discovers that service. Later scans don't update that asset type, even if the service's protocol changes in Kong Gateway. To change an asset's type, remove the asset, and let the scanner import it again.
