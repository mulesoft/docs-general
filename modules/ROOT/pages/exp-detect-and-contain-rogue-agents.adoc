...
* The *Rogue Agent Detection* policy monitors agent traffic and flags risky behavior for your review.
* The *Agent Kill Switch* policy blocks a flagged agent's access to the model proxy when you decide to contain it.

Both policies identify an agent from the same JWT claims, so the identity that Rogue Agent Detection flags is the identity that Agent Kill Switch blocks. Set the *Agent Identity Selector* to the same value in both policies.
...
+
.. In *Agent Identity Selector*, enter a CEL expression that identifies the calling agent from its validated JWT claims, for example, `claims.act.sub`. Use the same selector you configure on the Agent Kill Switch policy so the flagged identity is the one that gets blocked. This field is required.
...
// TODO: Confirm whether quarantining an agent from the Security page populates Killed Agent IDs automatically, or whether this list is maintained independently.
.. (Optional) Expand *Policy Details* to set a *Policy Label*, *Policy Version*, and *Application Conditions*.
. Select *Apply Policy*.
...
// TODO: This section's field labels (Select Alert Target, Specify Alert Configuration, Alert Metric, and so on) were not verified against the current UI. Confirm before publishing.
...
+
// TODO: Confirm the 90-day retention window. It is not surfaced in the current UI text.
...
// TODO: Confirm the action labels and the containment verb against the review window. The confirmation copy uses "block" and "restore access," so the button might read "Block" rather than "Quarantine agent."
+
* *Quarantine agent*
+
