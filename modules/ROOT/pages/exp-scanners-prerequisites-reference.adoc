...
*Setup:* xref:access-management::enabling-generative-ai.adoc#enable-einstein-anypoint[Enable generative AI in Anypoint Platform], accept the terms and conditions, and set a default Salesforce organization
...
| Data
a|
*Credentials:* Region; Informatica username and password. The scanner uses your Informatica Intelligent Cloud Services (IICS) username and password to authenticate against the selected region's pod and exchanges them for a session token.

*Region options:*

* North America (NA pod)
* EMEA (EM pod)
* United Kingdom (UK pod)
* Asia Pacific & Japan (AP pod)
* Canada (NA pod)

*Account:* Informatica Intelligent Cloud Services (IICS) account with Cloud Data Governance and Catalog (CDGC) enabled, with access to the catalog sources you want to scan

*Setup:* At least one CDGC catalog source registered (for example, a Snowflake connection) so that datasets, classifications, glossary terms, and quality metrics are available to ingest
