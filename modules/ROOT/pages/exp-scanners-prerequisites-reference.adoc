...
| Agent
a|
*Credentials:* Microsoft Entra app registration; tenant ID, client ID, client secret

*Account:* Microsoft tenant with a Microsoft Agent 365 license

*Permissions:* Admin consent for the `CopilotPackages.Read.All` Microsoft Graph scope

*Authentication:* Client-credentials *OAuth* only. Interactive *OAuth (Authorization Code)* authorization isn't supported yet.

*Setup:* Discovery reads declarative and custom engine agents from the Microsoft Graph Package Management API. Available in the Microsoft global cloud only.
...
* *Provider write permissions:* Grant the provider-specific *Write scope (policy apply)* listed for each API gateway provider in the Scanner Prerequisites Matrix.
