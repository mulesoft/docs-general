...
. Click *Create Alert*.
+
The alert creation wizard opens with three sections: Select Alert Target, Specify Alert Configuration, and Set Alert Delivery.
...
. Select an *Alert Metric* from the dropdown (for example, Request Count, Response Time, Response Code, or Policy Violation).
. Configure the alert condition:
** Choose a comparison operator (such as `>` for greater than)
** Enter the threshold value
** Select the time window (for example, 5 min, 10 min, 15 min)
+
The alert fires when the metric satisfies the comparison continuously for the selected duration.
