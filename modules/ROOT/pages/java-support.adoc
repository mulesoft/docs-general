= Java Support
:description: Learn how DataWeave supports Java types and enables seamless data transformation between Java objects and DataWeave formats in Mule applications.
:keywords: dataweave java support, java types, java objects, data transformation, mule applications, java integration, dataweave formats
...
In Mule 4.6.0 and later, the maximum supported version for running your applications is Java 17. In Mule 4.9.0 and later, you can compile any Mule project and additional dependencies for Java 17 or lower. In versions earlier than Mule 4.9.0, you must compile any Mule project and additional dependencies for Java 8.

When selecting a dependency (for example, `mssql-jdbc`), configure the version compatible with Java 8 or Java 17 in the `pom.xml`. For information about compatibility details, refer to the documentation of the dependency.
...
When running Mule applications on Java 17, you might observe heap memory patterns that differ from Java 8. For example, heap usage can remain at a higher baseline or not decrease immediately after load. This behavior is expected and reflects how Java manages memory. For more information, see https://help.salesforce.com/s/articleView?id=005132434&type=1[Mule heap memory pattern differences between Java 17 and Java 8].
