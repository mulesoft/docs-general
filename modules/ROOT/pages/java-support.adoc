...
When using TLS in a server with Java 11+, it is the server the one that determines the cipher preference, with the client supported ciphers list only used to filter, but never to determine priority. See https://bugs.openjdk.org/browse/JDK-8168261[JDK-8168261].

If you upgrade your server from Java 8 to Java 11+, this may cause a different cipher to be negotiated (depending on what was the supported ciphers list from the client). A change in cipher may incur in a peformance vs security trade-off.

There are various ways in which you can xref:mule-runtime::tls-configuration.adoc#optional-specify-protocols-and-cipher-suites[control the cipher selection] in case the defaults are not good for your needs.
