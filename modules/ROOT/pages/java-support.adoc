...
Java compatibility information is provided on the details page for connectors and modules in the *Asset versions* table. This is the most current compatibility information, as connectors are tagged as Java 17-compatible when they are released. 
...
To upgrade Anypoint Connectors provided by MuleSoft, with no custom code:
...
The Asset version detail table for connectors shows which versions of Java the connector is compatible with.
