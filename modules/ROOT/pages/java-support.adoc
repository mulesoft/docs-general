...
If you are using OOTB MuleSoft or automated policies and proxies, updating your policies to be compatible with Java 17 is straightforward. 
...
+
Policies that are compatible with Java 17 are listed in the *Automated Policies* > *Add a policy* page if you are using automated policies, or the *API Administration* > *<API-name>* > *Policies* > *Add a policy* page to apply policies at the individual API level. 
...
. Annotate the custom policy. 
. Upload the custom policy to API Manager. 
. Publish the custom policy to Exchange. 
. Update the API instance with the updated policy.
. In Runtime Manager, deploy the updated app to Mule Runtime 4.6 or later instance running on a Java-17 server.
  * xref:runtime-manager::cloudhub/deploy-mule-application-task.adoc[Deploying Apps to CloudHub]
  * xref:hosting::/ch2-deploy.adoc[Deploying Apps to CloudHub 2.0]
