...
+
Variables like `attributes`, `message`, or `error` have specific access APIs. Using the proper fields is required when using Java 17 because internal fields are no longer accessible. For example:

** `error.muleMessage` will need to be replaced by the proper Mule error field `error.errorMessage`
** `error.errors` will need to be replaced by the proper Mule error field `error.childErrors`
