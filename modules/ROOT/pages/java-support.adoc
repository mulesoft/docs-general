...
The majority of Mule applications currently in use can be updated, tested, and redeployed. However, any apps and connectors that are running custom Java code require additional work to certify those components. Before you upgrade your integration apps or Mule Gateway policies and proxies to Java 17, you must update all extensions, modules, and connectors used within those apps and policies to be Java 17-compatible.
...
Java 17 manages memory differently, with the goal of reducing latency and executing work more predictably. To achieve this, Java 17 adopts a different memory management approach. Although highly dependent on each application, Java 17 in some cases preferentially commits more data to heap memory, which results in higher initial memory usage. Java 17 also brings new Garbage Collection (GC) algorithms with changed defaults. The Mule runtime 4.6 LTS release provides, at a minimum, comparable or better performance for most apps as well as significant performance improvement (as high as 50%) for many apps. 

Because of this change in Java 17 memory management, to ensure optimal operation and effectively leverage these improvements, you must conduct xref:mule-runtime::tuning-test-validations.adoc[performance testing] and xref:mule-runtime::tuning-monitoring.adoc[monitoring] for your most common workloads.

[[runtime-fabric-performance]]
=== Runtime Fabric

If your workloads are running on the Runtime Fabric, you must allocate a minimum memory size of 1400mb of memory for 0.50 vCpu replica sizes. This is the minimum recommended size as the best configuration for memory allocation for most applications. With Runtime Fabric, even for the small fractionals with minimum memory, 50% of the total memory is allocated for heap. For example, a 450m request with a 550m limit CPU app with 1400mb will have a heap size of 700mb. 

After performance testing, some small adjustments, such as adding 100mb of memory or even subtracting memory, might be necessary, depending on your specific use cases.

[[cloud-hub-2-performance]]
=== CloudHub 2.0

With Java 17, Cloudhub 2.0 for small fractionals (micro offerings) continues to support the same applications supported with Java 8, within the bounds of limitations for CPU and heap memory size for the offering size. Features such as Apikit, Api Gateway policies, DataWeave, and so on, are supported with comparable or better performance in most cases on the micro offering. 

The total memory for the micro and nano offerings has increased from 1250mb to 1400mb. The heap size remains unchanged. For example, the micro offering 0.1 vCpu of a 450m request 550m limit CPU app has 1400mb of total memory and still has a heap size of 500mb. More native memory is provided for the CloudHub 2.0 managed offering versus Runtime Fabric to enable you to fully use the CPU and heap provided with the offering.

=== Garbage Collection Algorithms

MuleSoft is following the Java 17 garbage collection defaults. The following table shows which garbage collection algorithms are used for Mule runtime 4.6.0 or later. 

[%header%autowidth.spread]
|===
|Java version | vCores | Algorithm
| Java 8 | All | Parallel GC*
| Java 17 | < 2 |Serial GC
| Java 17 | >= 2 | G1
|===

*Using deploy flags, you can choose to override and use ‘CMS’ where there are specific application needs, however CMS was deprecated in Java 11. 

* Parallel GC 
+
MuleSoft Java 8 uses Parallel as the default garbage collection algorithm, which is a generational collector. This algorithm uses the standard young and old generation heap format. Oracle also refers to this algorithm as a “throughput collector”. 

* Serial GC
+
Serial GC is the Java 17 default for fractional and small vCore offerings. Serial is also a generational collector. Serial uses a single thread for GC, whereas Parallel can use multiple threads to increase garbage collection speed.

* Garbage-First (G1) GC
+
Used for larger offerings of >= 2 vCores. It serves multi-core machines with very large amounts of memory. Per Oracle, "it attempts to meet garbage collection pause-time goals with high probability while achieving high throughput with little need for configuration.”

For more information, see the xref:https://docs.oracle.com/en/java/javase/17/gctuning/parallel-collector1.html#GUID-DCDD6E46-0406-41D1-AB49-FB96A50EB9CE[Java 17 HotSpot Virtual Machine Garbage Collection Tuning Guide].
...
Upgrading your server from Java 8 to Java 11 and later can cause negotiation of a different cipher (depending on the supported ciphers list of the client), which can affect the trade-off between security and performance.

If you want to change the cipher's default values, see xref:mule-runtime::tls-configuration.adoc#optional-specify-protocols-and-cipher-suites[control the cipher selection].
...
Starting with Anypoint Studio (Studio) 7.17, if your Mule app is running on Mule runtime 4.6 or later, you can change the target JVM at the Studio project level to upgrade or downgrade the JVM. When you compile your app, Studio 7.17 offers real-time guidance and alerts you if there are any incompatible connector versions or project mismatches to prevent deployment failures.
...
When you deploy an app to CloudHub from Studio, Runtime Manager proactively detects mismatches between the project's Java version and the deployment environment. For example, if your project is built for Java 8 and your target environment is a Java 17 environment, Runtime Manager and Studio provides guidance to prevent deployment failures.
...
+
POJOs are required from DataWeave 6.0.0 on, and you must ensure that the POJOs have:
