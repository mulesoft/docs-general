...
Upgrading your server from Java 8 to Java 11 and later can cause a different cipher to be negotiated (depending on the supported ciphers list of the client). A change in cipher can affect the trade-off between security and performance.

If you want to change its default values, see xref:mule-runtime::tls-configuration.adoc#optional-specify-protocols-and-cipher-suites[control the cipher selection].
