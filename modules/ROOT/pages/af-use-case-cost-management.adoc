...
* Full cost visibility: See token consumption and spend by application, agent, user, and model in one place.
* Cost-optimized routing: Route each request to the right model at the right cost with semantic routing.
* Enforced budgets: Cap token or spend usage with Model Wallets that block requests when a budget limit is reached.
* Consistent governance: Apply the same policies, prompt protection, and automatic fallback across every provider.
* Cost attribution: Trace every token and dollar to the team, application, or agent that drove them.
