...
. To test the API deployment, send a request from Advanced Rest Client:
  ...
  +
  This URL will also be used later when you configure the proxy API.
  +
...
. For *Proxy app name*, which is the name displayed in Runtime Manager, enter *hello-world-greeting-proxy*, but append your last name or other unique string to ensure the proxy name is unique, and then click *Next*.
. Click *Select API from Exchange* if it's not already selected.
. In the *Select API* search box, enter `hello` and select *hello-world-api* from the drop-down. Note that you are working with the API, not the deployed app named `hello-world-greeting`.
...
+
image:api-led-select-server-exchange.png[Expanded more menu to show the list of available servers]
+
...
. Click *Shar*.
. In the *Share* dialog window, click *Public* and then select the API version to make available on the public Exchange portal.
+
...
+
image:api-led-rate-limiting-config.png["Rate limiting Limits configuration with number of requests 3, time period 1, time unit minute, and the Expose Headers checkbox selected"]
+
...
. Click the pencil icon to edit, and change the value in *Instance Label* to `Rate-limiting policy`.
. Click *Save & Apply*.
