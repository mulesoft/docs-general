...
. Verify the following values are selected:
  ...
  ** Implementation URI: `+http://hello-world-greeting.us-w1.cloudhub.io/`
...
To set the deployment configuration options and deploy the new proxy:

. In the Deployment Configuration section, enter *Runtime version* 4.2.2.
...
A progress window displays. You can view the logs as deployment progresses by clicking *Click here*.
...
THIS DOESN'T WORK. SOMETHING WRONG WITH CONFIG.
