...
within Studio, you can deploy it to the API to CloudHub. First, you will create an API proxy,
...
To add a rate limit policy to your deployed API, you must first add your API to API Manager, and create an API proxy.
...
. Enter `hello` and select `hello-world` from the drop-down. Note that you are working with the API, not the deployed app named `hello-world-greeting`.
. Verify the following values are selected:
  ...
  ** You can ignore TLS context since your API simply returns a hard-coded value.
  ** Path: `/`
...
. Click *Mocking Service* to open the drop-down menu.
...
. Click *Rate limiting*, select *1.3.5* or a later version if available, and click *Configure Policy* at the bottom of the dialog box.
