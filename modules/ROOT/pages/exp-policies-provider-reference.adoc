...
For Apigee, the connected Google service account must hold the following permissions for policy create, edit, and remove to work. They cover the read-import-deploy cycle every Apigee change performs:
...
At launch, Anypoint supports five universal use cases. Each is applied as the following native policy on each provider. These native policies are exactly the ones you can create and edit; all other recognized policies are view-only.
...
In addition to the universal use cases, Anypoint supports these previously supported native Kong policies: `acl`, `acme`, `basic-auth`, `header-cert-auth`, `ldap-auth`, `ldap-auth-advanced`, `mtls-auth`, `opa`, `tls-handshake-modifier`, and `tls-metadata-headers`.
