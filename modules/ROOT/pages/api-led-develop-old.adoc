...
image:canvas-tabs.png["Anypoint Studio canvas showing four tabs: one canvas tab and links to Configuration XML, Global Elements, and Mule Properties views", 500]
...
. Click the edit icon image:edit-module-version-icon.png["Pencil icon used to edit a module version in the API development interface"] next to the configuration name, and verify that the host is `0.0.0.0` and the port is `8081`.
...
+
image:set-payload.png["Mule canvas showing Set Payload transformer selected, with General configuration tab displaying Value field set to Hello World", 500]
...
+
image:api-led-assets-list-arrow.png["Anypoint Exchange Assets list page with an arrow highlighting the hello-world-app asset"]
+
...
. (Optional) Sign out of Exchange and search for the asset you just created. 
  ...
  +
  image:published-in-exchange.png["Exchange asset page showing version 1.0.0 details with description, tags, and lifecycle state fields visible"]
