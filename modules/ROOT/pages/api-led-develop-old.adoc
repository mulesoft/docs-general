...
image:canvas-tabs.png["Anypoint Studio canvas showing one open tab with links to Design, Message Flow, and Global Elements views", 500]
...
. Click the edit icon image:edit-module-version-icon.png["Pencil icon used to edit a module version in the API-led development interface"] next to the configuration name, and verify that the host is `0.0.0.0` and the port is `8081`.
...
+
image:set-payload.png["Anypoint Studio canvas with Set Payload connector selected and its General configuration tab showing Value and MIME Type fields", 500]
