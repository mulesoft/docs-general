...
image:empty-studio.png["Blank Anypoint Studio canvas showing the empty Package Explorer and canvas area with no open projects"]
...
+
image:new-project-dialog.png["The screenshot shows a New Project dialog in what appears to be an IDE or development tool, with a left navigation panel listing project types and a main content area displaying project configuration options including fields for project settings and template selection."]
...
image:canvas-tabs.png["Anypoint Studio canvas showing three navigation tabs labeled Message Flow, Global Elements, and Configuration XML above a flow editor area", 500]
...
+
image:link-to-published-api.png["Dialog showing link to API specification in Exchange with asset ID fields and version information visible", 600] 
. Click the link to view the project in Exchange:
+
image:api-led-assets-list-arrow.png["Anypoint Exchange Assets page showing a list with an arrow pointing to the hello-world-app entry in the search results"]
+
