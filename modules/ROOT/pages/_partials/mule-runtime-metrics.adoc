...
With standalone deployments, you can deploy your applications from the Runtime Manager cloud console to your Mule servers, and use Runtime Manager to manage them. It's a hybrid deployment option that gives you the best of both worlds: the flexibility of on-premises deployment with the convenience of cloud-based management. For more information, see xref:runtime-manager::deployment-strategies.adoc#hybrid-deployments[Hybrid Deployments] in the Runtime Manager documentation.

Hyperforce, Government Cloud, and Anypoint Private Cloud Edition don't support standalone deployments.

Standalone deployments show both managed and sideloaded applications on the usage reports. Managed applications are deployed using Anypoint Runtime Manager, while sideloaded applications aren't deployed using Anypoint Runtime Manager.
...
|Environment Type |Environment a Mule app is deployed to

| Cluster |Name of the cluster containing the replicas and worker nodes

|Cluster Capacity (Millicores) | Worker node capacity
|===
...
| CPU Limit | The maximum amount of vCPU the application can use. The CPU amount is shared on the worker node.
...
| Data Throughput (GB) | The total of inbound and outbound data in gigabytes (GB) transmitted by the Mule app. For more information about how this metric is billed, see xref:usage-metrics.adoc#data-throughput[data throughput].  
|===

// end::table[]

//// The tags from here to the end of the file are not in use as far as I know. 
// tag::flows[]
=== Mule Flows
