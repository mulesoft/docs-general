:description: Learn how partners and customers can configure, manage, and use MuleSoft solutions with step-by-step guidance for common tasks and workflows.
:keywords: mulesoft partner guide, customer guide, mulesoft configuration, partner management, anypoint platform, mulesoft setup, partner integration
...
+
If your custom connector is running on Mule 4.5.x or earlier, you canât run Java 17 tests that extend the `MuleArtifactFunctionalTestCase`, but you can still run Java 8 tests.
+
...
* Declaring the minimum Mule version
+
Two parent POMs are available for building extensions. Each one handles the minimum Mule version declaration differently.
+
** `mule-java-extension-parent` (recommended)
+
This is the recommended parent POM for all new extensions and for existing extensions that can migrate. It supports forward compatibility and requires you to explicitly declare the minimum Mule version in your project, giving you full control over which Mule runtime versions your connector supports. The `mule-extensions-archetype-maven-plugin` uses this parent when generating new extension projects.
+
For migration instructions, see xref:mule-sdk::forward-compatibility-setup.adoc#upgrading-existing-modules[Upgrading Existing Modules].
+
NOTE: Switching parent POMs for existing connectors may not be straightforward depending on the connectorâs complexity. If you must upgrade with minimal changes, you can continue to use `mule-modules-parent` as described below.

** `mule-modules-parent` (deprecated)
+
This parent POM is deprecated and remains available for existing connectors where switching to `mule-java-extension-parent` is not practical.
+
If you compile for Java 17, upgrade `mule-modules-parent` to version 1.9.0 or later. The connectorâs minimum Mule version is automatically set based on the parent version. For example, version 1.9.0 sets `minMuleVersion` to 4.9.0, ensuring compatibility only with Mule 4.9.0 or later.
+
If the `mule-modules-parent` version is earlier than 1.9.0, the bytecode level for the connector and its additional dependencies must be for Java 8. You canât use language features from Java 11 or Java 17.
+
Verify that `minMuleVersion` in `META-INF/mule-artifact/mule-artifact.json` reflects the correct version. See xref:mule-sdk::java-version-support.adoc#specify-java-compatibility[Specify Java Compatibility].

* Unsupported directives
+
Don't set `--add-opens`, `--add-exports`, or `--add-modules` directives. These directives are not allowed in CloudHub, Runtime Fabric, or on-premises. These directives also require managing them for each connector across all environments, and introduce a backward compatibility burden, complicating future Java upgrades and potentially enabling security vulnerabilities. If these directives are used, they are not covered under Support.
...
After you compile your custom connector, ensure you can run your custom connector with Java 17. Reviewing your code is important because some restrictions are flagged as warnings in Java 11 but are flagged as failures and canât be disabled in Java 17. For example, reflection is more restricted in Java 17, so you must review your code to ensure there are no illegal actions. 
...
image:general::single-build-pipeline.png[Example of single build pipeline]
...
JDeps is a tool for static code analysis that detects the usage of JDK internal APIs that are no longer available or accessible. For more information, refer to the https://wiki.openjdk.org/spaces/JDK8/pages/17957128/Java+Dependency+Analysis+Tool[OpenJDK wiki]. 

Add the JDeps Maven plugin to your custom connectorâs `pom.xml` file:
...
To set up your custom connectorâs `pom.xml` file to include the configuration:
...
If you use `mule-java-extension-parent` (recommended), the minimum Mule version is independent of the parent version and you control it directly in your project. For more information, see xref:mule-sdk::forward-compatibility-setup.adoc#upgrading-existing-modules[Upgrading Existing Modules].

If you use the legacy `mule-modules-parent`, the Mule runtime version determines the parent version. For example, Mule runtime 4.6.0 corresponds to `mule-modules-parent` 1.6.0. Minor versions maintain a correspondence, such as Mule runtime 4.1.0 with `mule-modules-parent` 1.1.0, Mule runtime 4.2.0 with `mule-modules-parent` 1.2.0, and so forth.

Mule runtime 4.6.0 and later supports Java 17. However, a connector can be compatible with both Mule 4.3.0 and Java 17 simultaneously. If your connector must be compatible with Mule 4.3.0, its `mule-modules-parent` version cannot exceed 1.3.0. You don't need to use `mule-modules-parent` 1.6.0 for your connector to be compatible with Java 17. Using `mule-modules-parent` 1.6.0 is specifically required to leverage other features from the Mule runtime 4.6.0 in the connector.
...
. If you havenât already, add the `munit-maven-plugin`. Replace the version with the `${munit.version}` parameter and replace `runtimeVersion` with the `${app.runtime}` parameter.
+
[source,xml,linenums]
----
	<plugin>
...
. Open a terminal window in the root of your Mule project and run the following command. This command displays the earliest versions of Mule runtime, MUnit, and the Mule Maven plugin that are compatible with Java 17.
+
[source,bash]
----
