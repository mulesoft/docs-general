...
Because reflection is more restricted in Java 17, API objects now require setters. Previously, API objects and plain old Java objects (POJOs) had default constructors and getters for all properties. Now, API objects and POJOs must also have setters so DataWeave can build outside the connector in the Mule app. Ensuring that API objects now require setters is crucial because it allows for more flexible and robust data handling. This change facilitates compatibility with Java 17's stricter reflection rules and DataWeave's requirements, making sure your connector can interact seamlessly within Mule applications.
