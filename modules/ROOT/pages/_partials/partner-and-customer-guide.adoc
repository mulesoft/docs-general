...
** JPMS
+
Adjust your code to work with the Java Platform Module System (JPMS). Ensure that your modules do not rely on introspection of JDK internal classes. For any connectors or classes that use JDK internals, replace them with DTOs and configure packages to be opened only when necessary for serialization. Update your codebase to adhere to JPMS requirements and avoid reflective access wherever possible.
...
----
. Run your MTF test to generate the coverage report. Use the `-Dtest=none` and `-DfailIfNoTests=false` flags when running the test to avoid including the JUnit and TITA tests in the coverage report.
+
...
The coverage percentage reflects the confidence line in which you can state your connector is Java 17 compatible. A key issue to avoid is illegal reflective access in your code. The only ways to detect this are to <<review-your-code>> or through testing. The higher the coverage percentage, the better your chances of catching Java 17 violations. Aiming for at least 80% coverage allows flexibility on simpler code, such as getters and setters, while ensuring thorough testing of all critical business logic.
