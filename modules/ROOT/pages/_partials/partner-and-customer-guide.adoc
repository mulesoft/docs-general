...
Because reflection is more restricted in Java 17, API objects now require setters. Previously with Java 8, it was enough for API objects and plain old Java objects (POJOs) to only have default constructors and getters for all DataWeave accessible properties. Now, API objects and POJOs must also have setters so DataWeave can write those properties without using reflection.
