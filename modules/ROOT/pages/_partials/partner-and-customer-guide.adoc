...
+
Two approaches are supported for declaring the minimum Mule version. 
+
** `mule-java-extension-parent` (recommended)
+
This parent POM provides full control over the minimum Mule version. New connectors should use this parent, and existing connectors can migrate to it. For migration instructions, see xref:mule-sdk::forward-compatibility-setup.adoc#upgrading-existing-modules[Upgrading Existing Modules].

** `mule-modules-parent`
+
This approach is still supported for compatibility. Be aware of this version-specific behavior:
+
*** For versions 1.9.0 through 1.10.x, the connector’s minimum Mule version is automatically set based on the parent version. For example, version 1.9.0 sets `minMuleVersion` to 4.9.0.
*** For version 1.11.0 and later, you must explicitly specify `min.mule.version` in your `pom.xml`. If you don't set this property, the `minMuleVersion` defaults to 4.1.1, which is incorrect for connectors compiled with Java 17. Add this property to your `pom.xml`:
+
[source,xml]
----
<properties>
...
----
+
*** For versions earlier than 1.9.0, the bytecode level for the connector and its additional dependencies must be for Java 8. You can’t use language features from Java 11 or Java 17.
+
