...
+
NOTE: Switching parent POMs for existing connectors may not be straightforward depending on the connector’s complexity. If you must upgrade with minimal changes, you can continue to use `mule-modules-parent` as described below.
...
+
If you compile for Java 17, upgrade `mule-modules-parent` to version 1.9.0 or later. The connector’s minimum Mule version is automatically set based on the parent version. For example, version 1.9.0 sets `minMuleVersion` to 4.9.0, ensuring compatibility only with Mule 4.9.0 or later.
+
