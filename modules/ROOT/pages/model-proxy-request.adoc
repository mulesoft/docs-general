...
* *OpenAI format*: Supports all LLM providers with multi-routing and fallback mechanisms. Supports both Chat Completions (`/chat/completions`) and Responses (`/responses`) endpoints.
* *Gemini format*: Native Gemini API format. Doesn't support multi-routing or fallback mechanisms.
* *Anthropic format*: Native Anthropic API format. Doesn't support multi-routing or fallback mechanisms.

All format examples are shown in the <<request-examples,Request Examples>> section below, where each example includes all four request format options.
...
Each example below shows four ways to make the same request:

* *Chat Completions Semantic Routing*: Uses the `/chat/completions` endpoint without specifying a model. The Model Proxy routes based on content.
* *Chat Completions Model Routing*: Uses the `/chat/completions` endpoint with a specific model (e.g., "gemini/gemini-3-flash-preview").
* *Gemini*: Uses the native Gemini API format with the `/models` endpoint.
* *Anthropic*: Uses the native Anthropic API format with the `/v1/messages` endpoint.
...
[tabs]
====
Chat Completions Semantic Routing::
+
[source,cli]
----
...
This request example extracts patterns into structured JSON to transform content into a clean JSON object for insertion into a database by using the `top_p` and `max_completion_tokens` parameters.
...
This request example validates the gateway's ability to handle large-payload "Expert" prompts. The example asks an AI Solution Architect to design a migration strategy from legacy systems to Salesforce using MuleSoft Anypoint Platform with an API-led connectivity approach.
