...
Use Akamai API Security scanner to correlate security findings and risk signals with services that already exist in *Portfolio* catalogs. The scanner doesn't import or register third-party services. It applies Akamai security correlation so teams can review risk scores, findings, and incidents in one governance workflow for APIs and MCP services.
