...
link:https://anypoint.mulesoft.com/designcenter/#/projects["Take me to API Designer^" role="button-primary"]
+
image:api-spec1.png["Annotated API specification editor showing RAML code with response types, query parameters, and examples in a dark-themed interface"]
+
. Click the Documentation icon if the Documentation panel isn't already open.
+
image:api-documentation-icon.png["Documentation icon showing a book with a magnifying glass overlay, representing API documentation search or reference"]
+
...
+
image:try-it.png["Try It button highlighted in the API Console panel with a dark background and white text", 500]
...
+
image:successful-test1.png["Successful API test results showing 200 status code, response time, and green checkmark indicators in Anypoint Platform", 500]
...
+
image:api-mocking-service-configuration-icon.png["Mocking service configuration icon button in the API design toolbar, showing a toggle/switch UI control"]
...
+
image:publish-to-exchange1.png["Publish API to Exchange dialog showing API Name, Asset Version, API Version fields and Save button", 500]
+
