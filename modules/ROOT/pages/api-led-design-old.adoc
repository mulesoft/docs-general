...
link:https://anypoint.mulesoft.com/designcenter/#/projects["Take me to API Designer^" role="button-primary"]
+
image:api-spec1.png["RAML API specification editor showing a completed spec with resource definitions, HTTP methods, and response types in the left panel"]
+
. Click the Documentation icon if the Documentation panel isn't already open.
+
image:api-documentation-icon.png["Book icon with an open page design, representing API documentation in API Designer"]
+
. Look for the label *API endpoints*. You can see the endpoint you defined. HTTP requests appear in green boxes.
+
image:get-button.png["GET button highlighted in blue in the API designer interface", 500]
. Click *GET* to display the GET request and more information about the specification.
+
image:code-response1.png["DATAURL API response code panel showing HTTP status 200 with a JSON response body containing example fields", 500]
...
+
image:try-it.png["Try It button in the upper right corner of an API page in API Designer, enabling interactive API testing", 500]
...
+
image:successful-test1.png["API test result panel showing a 200 OK status response with response body, headers, and execution time displayed", 500]
...
+
image:api-mocking-service-configuration-icon.png["Green circular icon with a white play button triangle, used to activate the API mocking service"]
...
+
image:publish-to-exchange1.png["Publish to Exchange dialog showing fields for Name, Main file, Asset version, API version, and checkboxes for options", 500]
+
