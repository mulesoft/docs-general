...
link:https://anypoint.mulesoft.com/designcenter/#/projects["Take me to API Designer^" role="button-primary"]
+
image:api-spec1.png["RAML API specification editor showing version, title, baseUri, and GET method with query parameters and responses defined"]
+
. Click the Documentation icon if the Documentation panel isn't already open.
+
image:api-documentation-icon.png["Book icon with an open page and lines suggesting text, used as API Designer documentation link indicator"]
+
. Look for the label *API endpoints*. You can see the endpoint you defined. HTTP requests appear in green boxes.
+
image:get-button.png["GET button highlighted in blue in the Anypoint Platform API designer interface", 500]
. Click *GET* to display the GET request and more information about the specification.
+
image:code-response1.png["API response panel showing a 200 status response with a body field containing example JSON output", 500]
...
+
image:try-it.png["Try It button displayed in blue in the API designer interface", 500]
...
+
image:successful-test1.png["API test result panel showing Status 200, response time 576ms, and a green success indicator with response body details", 500]
...
+
image:api-mocking-service-configuration-icon.png["Mocking service toggle icon in enabled state, shown as a circular button with a power symbol"]
...
+
image:publish-to-exchange1.png["Publish to Exchange dialog showing asset name, API version, asset version fields, and a Publish button", 500]
+
