...
Semantic services compare incoming requests to defined prompt topics to route the request to the model path that best matches it or block requests that match deny list topics.
...
+
Scales up to 2000 utterances per topic.
...
. Select *Basic Scale*.
+
Basic scale supports up to 6 topics and 10 utterances per topic. Managed via internal Model Proxy configuration.
. Configure the embedding connection parameters:
** *Environment*: The environment to use for the semantic service.
...
+
If creating new prompt topics for an advanced scale semantic service, it is necessary to redownload and run the vector script in your database again.