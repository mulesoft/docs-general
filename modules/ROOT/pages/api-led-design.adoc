...
<2> A single `dataType`, `greeting`, with a single property, `todays-greeting` (today’s greeting message). 
<3> HTTP response
...
. Review the *Security* method. *Basic Authentication* is the only option.
. Click *Try it*.
. Enter values in the Authorization and Parameters tab: 
  ...
  .. In the Parameters tab, enter a value for *todays-greeting*: `Hello World`. 
