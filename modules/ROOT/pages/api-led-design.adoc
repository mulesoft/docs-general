= Build an API: Design an API Specification
...
link:https://www.mulesoft.com/exchange["Take me there^", role="button-primary"]
 +
  ...
  .. Select *REST APIs* from the *Type* dropdown to display only REST API specifications.
  .. Click on any specification to see the data types and the HTTP requests defined for the API. 
+
* Look at the Exchange for your organization (account).
+
link:https://anypoint.mulesoft.com/login["Take me there^", role="button-primary"] 
  +
  .. Log in to Anypoint Platform
  .. Click *Discover and share* under *Exchange*. Logging in changes the view to your organization's private Exchange.
  .. Click on any specification to see the data types and the HTTP requests defined for the API.
...
. Open Anypoint Platform if it isn't open already:
+
link:https://anypoint.mulesoft.com/login/["Take me there^", role="button-primary"]
...
. The API Designer editor displays a sample RAML definition. Replace all the text with RAML that specifies the API:
...
<2> A single data type, `greeting`, with a single property, `todays-greeting` 
<3> An HTTP success response
<4> An HTTP error response

We don't authentication to keep this tutorial simple.
...
. Open `hello-world.raml` in Design Center if it isn't already open:
+
link:https://anypoint.mulesoft.com/designcenter/#/projects["Take me there^" role="button-primary"]
...
. Look for the label *API endpoints*. You can see the endpoints you defined. HTTP requests appear in green boxes.
. Click *GET* to display the GET request.
. Click *Code examples* to review samples for each protocol.
. Click *Security* to review authentication and parameter information. *Basic Authentication* is the only option for this API; normally others would be available here.
. Click *Try it*.
