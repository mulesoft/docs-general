= Step 4. Validation and Unit Tests
...
Our API is very simple, so our testing will be simple: testing that the size of the payload is within an acceptable range.

Add the *Validate size* operation to the `hello-world` project:

. In the `hello-world` project in Studio, open `hello-world.xml` if it isn't already open.
. In *Mule Palette*, select *Add Modules > Validation* and drag it to the drag and drop area in the left side of Mule Palette. 
. From the list of operations displayed, drag *Validate size* to the canvas and drop it to the right of *Set Payload*, inside *hello-world-main*.
. Double-click *Validate size* and then set these values in the *General* tab:
  ...
  .. *Min*: `6`
  .. *Max*: `30`
  .. *Error options > Message*: `The specified message is either too small or too large.`
...
You don't need to stop the project to make these changes.
...
were automatically created in `hello-world.xml` for `hello-world-main`:
...
Delete all the error handlers except *APIKIT:BAD_REQUEST*.

. For each error handler to delete, right-click in the *On Error Propogate* box at the top of the error handler.
. Select *Delete*.
