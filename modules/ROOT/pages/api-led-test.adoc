...
. In *Mule Palette*, select *Add Modules > Validation* and drag it to the Studio canvas.
+
image:validation-module.png[View of Mule Palette with the Validation module selected]
. From the list of operations displayed, drag *Validate size* to the Studio canvas and drop it to the right of *Set Payload*, inside the endpoint module.
+
...
. Click the curved yellow arrow again, and notice that that an error message you defined is now in the *error* object in the debugger pane, and a red dotted-line shows in the Validate size operation.
+
