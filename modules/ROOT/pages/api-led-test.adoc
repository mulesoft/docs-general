...
Your API is very simple, so your testing will be simple. You'll test whether the size of the payload is within an acceptable range.

Add the *Validate size* operation to the `hello-world-2` project:

. In the `hello-world-2` project in Studio, open `hello-world.xml` if it isn't already open, and make sure you are viewing the *Message Flow* tab of the canvas.
. In *Mule Palette*, click *Add Modules > Validation* and drag it to the *Mule Palette*.
+
image:validation-module.png[View of Mule Palette with the Validation module selected]
. From the list of operations displayed, drag *Validate size* to the Studio canvas and drop it inside the endpoint module to the right of *Set Payload*.
+
...
. Right-click in the canvas and select *Run project hello-world-2*.
. After you see the console message indicating `hello-world-2` is deployed, send a request using Advanced Rest Client or a similar tool:
+
[source, xml]
----
...
----
+
Your request client should report response headers containing `200 OK` and `Today the greeting is Hello.`. This demonstrates the expected behavior when the greeting is within the limits you set in the Validate size operation.

. Right-click the canvas and select *Stop project hello-world-2*.
...
. In the `hello-world-2` project in Studio, open `hello-world.xml`.
. Click *Set Payload*, and then change the value to `Hi.`.
. In *Mule Palette*, select *Favorites > Logger*, and then drag and drop the logger in between *Set Payload* and *Validate size*. The logger will help stop the process to help debugging. No configuration of the logger is needed.
...
. Right-click the canvas and select *Debug project hello-world-2*. If you are asked to open the Mule Debug perspective, select the *Remember my decision* checkbox and click *Yes*.
+
...
. Click the curved yellow arrow again, and notice that an error message you defined is now in the *error* object in the debugger pane, and a red dotted-line shows in the Validate size operation.
+
...
. Right-click on the canvas and select *Stop project hello-world-2*.
...
. In Studio *Mule Palette*, select *Core > On Error Propagate*, and then drag and drop the error handler on *Error handling* in *get:\greeting:hello-world-2-config*.
+
...
. Right-click in the canvas and select *Debug project hello-world-2*.
...
. When you have stepped all the way through and looked at the response in Advanced Rest Client, you'll see that a *500 Server Error* is returned along with the error message.
...
Next, you'll publish the revised API.

== Step 4.3: Publish Your Revised API

Make your improved API available on Exchange:

. From *Package Explorer* in Studio, right-click the `hello-world-2` project.
