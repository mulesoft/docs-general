...
. In *Mule Palette*, select *Favorites > Logger*, and then drag the logger in between *Set Payload* and *Validate size*. The logger will help stop the process to help debugging. No configuration of the logger is needed.
. Right-click *Set Payload* and select *Add breakpoint*.
. Repeat the previous step for *Logger* and *Validate size*.
. Right-click the canvas and select *Debug project hello-world-app*. If you are asked to open the Mule Debug perspective, select the *Remember my decision* checkbox and click *Yes*.
+
...
. In Studio *Mule Palette*, select *Core > On Error Propagate*, and then drag and drop the error handler on *Error handling* in *get:\greeting:hello-world-app-config*.
+
