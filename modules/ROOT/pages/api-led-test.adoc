...
. In *Mule Palette*, select *Favorites > Logger*, and then drag the logger in between *Set Payload* and *Validate size*. The logger stops the process to help debugging. No configuration of the logger is needed.
...
. In the canvas, click *Set Payload* to open the *Set Payload* node in the *Mule Debugger* above the canvas. 
...
. When you reach the *APIkit Router* in the message flow, you can see the error message replaces the original payload `Hi.`.
+
image:payload-response.png[Breakpoint at APIkit Router showing error message in payload]
. After you step all the way through and look at the response in Advanced Rest Client, you'll see that a *500 Server Error* is returned along with the error message.
