...
:description: Explore key MuleSoft and Anypoint Platform terms and definitions to understand core concepts used across integration and API development.
:keywords: mulesoft glossary, anypoint platform terms, integration concepts, api terminology, mule definitions, platform vocabulary
...
These layers work together to protect both the application network and the networkâs individual nodes by controlling access to APIs, enforcing policies, and proxying all inbound or outbound traffic to mitigate external threats and attacks.
...
message receiver::
A Java class used by a connector to read the incoming data, package it as a message, and pass it to a service componentâs inbound router. The message receiver can use a transformer if necessary to convert the data.
...
MuleÂ® runtime engine (MuleÂ® or Mule)::
