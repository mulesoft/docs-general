...
Providers::
The external cloud platforms connected to the enhanced experience to enable automated service discovery and import, or to connect an external vault. Configure providers under *Platform* > *Providers*. The *Self-managed* category lets you create a scanner that you push service definitions to instead of connecting to an external platform.

Scanner::
A configured link that discovers services and registers them in the matching Portfolio catalog. A provider scanner connects to a supported cloud provider and pulls services in, running on a schedule or on demand. A self-managed scanner works in the opposite direction: your own pipeline pushes service definitions to it. A scanner is distinct from an external vault, which syncs secret metadata rather than discovering services.

Self-managed scanner (custom scanner)::
A scanner that registers services from your own build pipeline instead of connecting to a third-party provider. You create it once (in the UI under *Platform* > *Providers* > *Self-managed*, or with the scanner-configuration API), then push service definitions to it from a script, CI/CD job, or any REST client. Sealing the workspace runs a scan that reconciles the pushed services into the matching Portfolio catalog. See xref:exp-scanners-build-custom.adoc[].
