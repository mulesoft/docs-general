...
+
image:validate-size.png["Validate Size module card dropped into a GET flow in Anypoint Studio's Mule Palette canvas view"]
...
+
image:next-toggle.png["Blue circular button with a right-pointing arrow icon used to advance to the next toggle point in a flow", 500]
+
...
+
image:validation-error.png["Debugger panel showing a Validate size component error with red error indicator and error details in the message panel", 500]
...
+
image:on-error.png["On-error-propagate scope in Anypoint Studio canvas with HTTP Listener, Logger, and error handler components configured"]
