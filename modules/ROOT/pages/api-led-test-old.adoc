...
+
image:validate-size.png["Validate Size module dropped into a GET flow in Anypoint Studio canvas with connected processors"]
...
+
image:next-toggle.png["Blue circular arrow icon pointing right, used as a navigation control to advance to the next toggle point", 500]
+
...
+
image:on-error.png["On-error-propagate scope added to GET flow, showing Error Type field set to ANY and Description field empty"]
