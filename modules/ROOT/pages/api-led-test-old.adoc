...
+
image:configure-validate.png["Validate size configuration panel showing a tab interface with input fields for size validation settings"]
...
+
image:on-error.png["I can see a MuleSoft Studio configuration screenshot showing an error handler being added to a GET flow with configuration settings visible.

**Alt text:**

Error handler configuration panel in Anypoint Studio showing a GET flow with On Error Continue component, Error Type field set to ANY, and Enable Notifications checkbox unchecked"]
