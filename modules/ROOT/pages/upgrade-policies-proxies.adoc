= Upgrade Java for Policies and API Proxies 
...
To upgrade API-level xref:gateway::policies/policies-availability-by-gateway.adoc[MuleSoft included policies] to Java 17:
...
. Deploy Mule runtime engine 4.6 in a new server running on Java 17 and start the server. For more information, refer to xref:runtime-manager::servers-create.adoc[].
...
. Deploy Mule runtime engine 4.6 in a new server running on Java 17 and start the server. For more information, refer to xref:runtime-fabric::proxy-deploy-runtime-fabric.adoc[].
. Select the new target you created running on Java 17 and in API Manager, go to *API Administration > Settings*.
. xref:api-manager::edit-api-endpoint-task.adoc[Edit the API instance's configuration settings]:  
 .. In *Runtime Channel*, select the runtime channel to use.
 .. In *Version*, select *4.6.0*.
