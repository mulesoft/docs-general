...
The Usage Reports page supports pricing packages for xref:https://www.mulesoft.com/anypoint-pricing[integration]. When your organization opts in to a usage-based pricing plan, such as the Anypoint Integration Advanced package, you must redeploy your apps. After redeploying your apps, your usage reports are shown. 
...
Application deployment using the Mule Maven plugin is not yet supported. To deploy your apps, use the Runtime Manager UI and the Application Manager APIs with the new field in `target.deploymentSettings` called `instanceType`.
