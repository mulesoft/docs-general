...
:description: Explore MuleSoft pricing models, plans, and licensing options to find the right solution for your integration needs.
:keywords: mulesoft pricing, licensing, pricing plans, mulesoft costs, subscription, integration platform pricing, anypoint platform pricing

The Usage Reports page supports pricing packages for https://www.mulesoft.com/anypoint-pricing[integration]. When your organization opts in to a usage-based pricing plan, such as the Anypoint Integration Advanced package, you must redeploy your apps. After redeploying your apps, your usage reports are shown. 

For CloudHub and CloudHub 2.0, apps are automatically redeployed during the monthly runtime patch update cycle. For more information, see xref:runtime-manager::cloudhub-app-runtime-version-updates.adoc[] and xref:cloudhub-2::ch2-patch-updates.adoc[].
...
To understand the technical aspects of metering and pricing concepts within Anypoint Platform, see xref:general::usage-metrics.adoc[] documentation.
...
[NOTE]
This limit doesn't impact applications configured with xref:cloudhub-2::ch2-configure-horizontal-autoscaling.adoc[]. You can continue to deploy applications if Horizontal Pod Autoscaling is enabled even if you reach the global resource limits.
