...
When a caller sends a request, the proxy matches the client ID and claims to a wallet and counts the request against that wallet's budgets. Matching a wallet attributes usage. It does not grant or deny access to the proxy.
...
Budgets depend on model costs. To track budget usage accurately, configure the cost per token for each model on the *Models* page. Models without configured costs default to zero and don't count against the budget. See xref:exp-models-manage-costs.adoc[Managing Models and Model Costs].
