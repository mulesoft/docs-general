...
NOTE: Organizations without an IdP configured can't use model wallets. Those organizations continue to use existing access methods.
...
. From the actions menu for *DataWeave Headers Transformation*, select *Disable Policy*.
. From the actions menu for *Client ID Enforcement*, select *Disable Policy*.
. Select *Apply Policy*.
...
. Select *Apply Policy*.
. Select *Reorder Instance Policies*.
. Move *JWT Validation* to the second position.
+
The xref:gateway::policies-included-cors.adoc[CORS] policy remains first. JWT Validation in the second position validates the token before later policies read the claims.
. Select *Save Order*.
. From the actions menu for *LLM Proxy Core Policy*, select *Edit Configuration*.
. In *Client ID*, enter:
+
----
...
+
The Client ID is read-only. Wallet selection is driven by matching the caller's JWT claims against each wallet's required claims, with the highest-priority matching wallet winning. The Client ID is used only as a tie-breaker: when a caller matches several wallets tied on the best priority, sending the Client ID as the optional `x-mw-client-id` request header selects the tied wallet whose Client ID equals that value. If the header is absent or matches none of the tied wallets, the tied wallet whose Client ID sorts first alphabetically is used.
...
* xref:gateway::policies-included-dataweave-headers-transformation.adoc[]
