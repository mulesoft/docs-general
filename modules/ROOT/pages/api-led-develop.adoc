...
Now that you've created an API specification, use it to create an API.
...
You are going to deploy first to Mule in Studio, then, when the API is complete, you'll deploy it to Mule in CloudHub.
...
. Open Studio, and then click *Anypoint Studio > Settings*.
...
In this step, you'll create the project.
...
. In the dialog box, provide the following values:
  ...
  ** Select the *Import a published API* tab.
    ...
    ... Click *Finish*.
. Verify that the checkbox *Scaffold flows from these API specifications* is selected. This ensures that Studio adds default error handling and other helpful items to your project.
. Click *Finish*.
. Studio creates a `hello-world.xml` XML file and opens it in the canvas *Message Flow* view, showing the basic elements you'll need for the API interface:
...
* A console, *hello-world-2-console*, which you won't use for this project
* The GET request to endpoint `/greeting`, where you will do the bulk of your configuration.
...
Verify the values set in the Listener at the top of your flow in the canvas:
...
. Click *OK* to close the configuration. This leaves you in the *Global Configuration Elements* tab.
. Click *Message Flow* to return to the previous tab. General configuration values for Listener should still be displayed.
. Verify that *Path* is set as */api/**. This best practice ensures that all endpoints will be formed like `base-URI/api/endpoint`.

In the next step, you'll create business logic and configure the elements you need for this API.
...
. Specify the logic that displays the data sent in a response from the hello-world API:
...
image:set-payload.png[View of canvas and configuration dialog for Set Payload connector, 500]
  .. Below the canvas, in the *General* tab, click the *fx* (function) button to deselect it. For this tutorial, you are hard-coding the value.
...
You have created a project that contains a simple Mule app that you can deploy to expose your API.
...
*******************************************************************************************************
* hello-world-2                                  * default                        * DEPLOYED           *
*******************************************************************************************************
----
...
. Right-click in the canvas and select *Stop project hello-word-2*.
...
. In the *Parent business group* dropdown, select a business group you are an administrator for.
...
. In Studio, from Package Explorer, right-click the `hello-world` project.
...
. Enter values for your project:
  ...
  ** For *Business group*, select the *My Top Business Group* business group you created. You can't use the root business group, which appears by default in this field. If your credentials have expired, click *Add account* to add them again.
...
. In the *Asset Versions* section, click the three vertical dots and select *Version detail*. Notice that HTTP Connector, Sockets Connector, and APIKit module are listed as dependencies.
. After you have explored the published asset on Exchange, return to Studio and click *OK* to close the open dialog windows.
...
You will publish the updated version after it is complete.
