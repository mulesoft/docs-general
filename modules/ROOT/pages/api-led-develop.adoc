...
. Click *Sign in*, then close the pop-up window.
...
. In the dialog box, provide the following values:
  ...
  ** Select the *Import a published API* tab.
    ...
    ... Click *GitHub RAML* to select it.
...
. Click *Finish*.
. Studio creates an XML file, `api.xml` and opens it in the canvas *Message Flow* view,
