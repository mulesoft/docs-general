...
You've created an API specification, now use it to create the API.

In MuleSoft, developers package an API in an application that is deployed to a Mule runtime engine (Mule)
in one of several environments, called targets. One target is the Mule embedded in Studio, which you use for
development and testing. Another target, CloudHub, is managed by MuleSoft and is cloud based.

This tutorial shows you how to deploy first to Mule in Studio. When the API is ready, you'll deploy it to CloudHub.
...
. In the dialog box, provide the following values:
  ...
  ** Select the *From Exchange or Maven* tab.
    ...
    ... Click *Finish*.
. Scroll down to verify that the checkbox *Scaffold flows from these API specifications* is selected. This ensures that Studio will add default error handling and other helpful items to your project.
. Click *Finish*.
...
. Click *Finish* to close the configuration. This leaves you in the *Global Elements* tab.
...
. Specify the logic that displays the data sent in a response from hello-world API:
  .. Click the Set Payload connector you just added to the flow.
+
