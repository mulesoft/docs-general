...
+
On some providers, notably Apigee, the name is the policy's unique identity on the gateway, not just a label. Choose it deliberately: the name is required at creation and you can't change it later. See <<policy-naming-and-identity>>.
...
While an operation is in progress on an instance, you can't apply other policies on that instance until the running operation finishes. Wait for it to complete. You can track progress in the *Activity log*. This is related to how providers lock during operations. See xref:exp-policies-provider-reference.adoc#instance-locking-during-operations[Instance Locking During Operations].
