...
For select customers in the US control plane, MuleSoft offers a pricing and packaging model for Anypoint Platform that alots a number of Mule flows, Mule messages, and data throughput (measured as network I/O bytes). Not all types of Mule flows and messages count toward the allotments in a package. 

== Metrics Collected in Anypoint Platform

Anypoint Platform captures usage metrics across products, but not all metrics are billable or are reflected in usage reports.
