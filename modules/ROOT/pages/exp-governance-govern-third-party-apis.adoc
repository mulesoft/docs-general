...
. *Verify Discovered APIs*: Go to *Portfolio Overview* and verify that discovered APIs appear with the provider name in the *Provider* column. APIs are automatically added to the portfolio as they are discovered.

. *Create Governance Strategy*: Create a xref:exp-governance-create-strategy.adoc[governance strategy] and select the control rules or automated policies to apply. The strategy evaluates conformance for all in-scope APIs, including those discovered from third-party providers.

. *Monitor Conformance*: Track conformance status for provider-hosted APIs through the governance strategy dashboard. Use the *Provider* filter to focus on a subset of your cross-gateway environment.
...
. Review the conformance summary. The *Provider* column shows which platform hosts each API.
. Use the *Provider* filter to focus on conformance for a specific provider.
