...
[NOTE]
====
Amazon, Microsoft, and HashiCorp also support registering an external vault instead of a scanner. A vault syncs secret metadata rather than discovering services, so it doesn't populate a *Portfolio* catalog. For the vault registration flow, see xref:exp-vaults-manage.adoc[].
====
