...
As mentioned previously, you can have a single build pipeline that runs against all supported Java versions. You can also choose to set up another temporary build pipeline for Java 17 so your main build pipeline doesn't become unstable. After you upgrade to Java 17, you can discard the temporary build pipeline and converge on your main build pipeline.
...
. Ensure the following MTF dependencies are set in your custom connector’s `pom.xml` file:
+
* munit 3.1.0
...
+
. These MTF dependencies have a minimum Mule version of 4.3.0. To ensure your MTF tests do not validate against Mule runtime versions earlier than 4.3.0, add this to the munit-plugin configuration of your custom connector’s `pom.xml` file: 
+
[source,xml,linenums]
----
<configuration>
