...
Work with an Anypoint Platform admin to ensure that all <<before-you-begin, prerequisite configuration>> is done in the Anypoint Platform organization from which you want to sync APIs. The following are required before you can being using API Catalog in Salesforce: 
...
* An Anypoint Platform and Salesforce org connection is established and the API Catalog feature is enabled in Anypoint Platform Access Management. See xref:access-management::trusted-salesforce-org.adoc[].
...
. From Salesforce Setup, in the Quick Find box, enter API Catalog and select it.
. Click *Sync APIs*.
+ 
