...
* A consumer endpoint with an HTTPS URL that specifies a proxy application’s address for consumers to use for sending requests. To configure the consumer endpoint for Mule Gateway API instances, see xref:api-manager::create-instance-task-mule.adoc[].
...
* A *Consumer Endpoint* with an HTTPS URL in API Manager that is used to run the API
