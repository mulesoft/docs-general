...
. Start your application by clicking the image:icon-start-debug.png["",15,15] (*Run*) icon in the debug toolbar.
...
You ran the API locally and tested it with Postman, then hardened it by adding a Choice router and Raise Error component to validate the `name` parameter. You configured a Logger to record each request, then used Anypoint Code Builder's debugger — breakpoints and the Variables panel — to pause execution and inspect the Mule message at each step.
