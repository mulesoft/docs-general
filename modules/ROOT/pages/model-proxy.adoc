...
Model Proxy has these two types:
...
* *Connection*: A connection configuration that stores credentials and metadata for connecting to a specific LLM provider. The Enhanced Experience displays connections as a single-route proxy. Connection configurations are managed by external projects, such as an Agent Network projct.
