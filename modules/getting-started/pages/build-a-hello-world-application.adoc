...
+
Studio automatically wraps the connector with a flow, so you don't have to manually create one. There will be a red error dot overlaid on the HTTP Listener because you have not configured it yet.
+
...
+
The properties window opens, with Path populated with " / " with *Connector Configuration* populated with a reference to the global element you just created.
+
. In *Path*, enter " / ".
